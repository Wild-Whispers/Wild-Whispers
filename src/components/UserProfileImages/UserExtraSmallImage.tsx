...
export default function UserExtraSmallImage({ user, onClick }: { user: User, onClick?: () => void }) {
        ...
        ">
            {
                ...
                <div className="flex justify-center items-center w-full h-full rounded-lg text-6xl font-semibold bg-sky-700">
                    <p className="text-xs font-semibold select-none">{user.userName.charAt(0).toUpperCase()}</p>
                </div>
            }
        </div>
    );
